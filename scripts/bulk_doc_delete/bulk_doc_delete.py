...
Outputs formatted json with format \n\
    ...
    parser_h2o = subparsers.add_parser('h2o', help='Authentication using the h2o module (requries h2o to be setup prior to use)')
...
def main(argv: Namespace = None):
    ...
    if args.delete_younger == None and args.delete_older == None:
        raise SystemExit("|  EXITING   |  Please include either \"--delete_younger\" or \"--delete_older\" in command, please.")
    if args.delete_younger != None and args.delete_older != None:
        raise SystemExit("|  EXITING   |  Illegal inclusion of both \"--delete_younger\" and \"--delete_older\" parameters. Remove one, please.")
...
def delete_document_sets(args: Namespace, ACCESS_TOKEN: str):
        ...
        if project_dict["displayName"] in projects_to_delete_from or args.filtering == "All projects":
            ...
            for document_dict in document_get.json()["documentSets"]:
                ...
                if  args.delete_younger == None and document_age > args.delete_older:
                    ...
                    try:
                        annotation_get = get(url = f'{args.mlapi_url}/projects/{project_uuid}/documentAI/annotationSets:search',
                                             headers = default_headers)
                    except requests.exceptions.HTTPError as err:
                    ...
                    document_hierarchy[project_dict["displayName"]][document_dict["displayName"]] = {
                        "annotationSets":[{annotation_set["displayName"]:""} for annotation_set in dependencies_get.json()["dependencies"] if annotation_set["artifactName"][-1] == "1"],
                    ...
                    for annotation_dict in annotation_get.json()['annotationSets']:          
                        for i, anno_dict in enumerate(document_hierarchy[project_dict["displayName"]][document_dict["displayName"]]["annotationSets"]):
                            if list(anno_dict.keys())[0] == annotation_dict["displayName"]:
                                
                                document_hierarchy[project_dict["displayName"]][document_dict["displayName"]]["annotationSets"][i][annotation_dict["displayName"]] = annotation_dict["status"]
                                
                                if annotation_dict["status"] != "ARTIFACT_STATUS_AVAILABLE":
                                    print(f"|  WARNING   |  \"{annotation_dict['displayName']}\" reports it\'s status as \"{annotation_dict['status']}\"")
                                break
                    if args.preview == False:
                        print(f'|  DELETING  |  Deleting "{document_dict["displayName"]}" from "{project_dict["displayName"]}" with age {np.round(document_age,2)} days')
                        
                        try:
                            document_delete = delete(url = f'{args.mlapi_url}/{document_dict["name"]}:bulkDelete',
                                                     headers = default_headers)
                            document_delete.raise_for_status()
                        except requests.exceptions.HTTPError as err:
                            raise SystemExit(f'|  EXITING   |  DOCUMENT_DELETE CODE {document_delete.status_code}: {err}')
    ...
    if args.save_preview != None:
        if '.json' not in args.save_preview:
            print(f"Document Hierarchy could no be saved. Please specify a \".json\" file for --save_preview {args.save_preview}.")
        else:
